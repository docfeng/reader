...
var book = function () {
	...
	var list = {
		...
		format: function format(html, url) {
	...
	var page = {
		multi: function multi(url) {
			var t = this;
			return t.read(url).then(function (txt) {
				return txt;
			}).catch(function () {
				return t.get(url).then(function (txt) {
					t.save(url, txt);
					return txt;
				});
			});
	...
	var search = {
		...
		checkCharset: function checkCharset(url) {
		...
		getRealPath: function getRealPath(url) {
			return http.get(url, {
				cors: true,
				corsUrl: realURL,
				xml: true
...
book.search.get("唐残").then(function (arr) {
	...
	book.search.getRealPath(url).then(function (url) {
		return book.list.get(url).then(function (arr) {
			var url = arr[0][0];
